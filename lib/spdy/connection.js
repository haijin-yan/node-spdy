...
Connection.prototype._handleSynStream = function handleSynStream(frame) {
  ...
  // Associate streams
  if (associated) {
    console.log("ELE-LL new Stream and add Stream", frame.id, " do not add to pushes!");
...
Connection.prototype._removeStream = function removeStream(stream) {
  var state = this._spdyState;
  var id = stream._spdyState.id;
  var associated = stream._spdyState.associated;
  ...
  if (associated > 0) {
    var assoc_stream = state.streams[associated];
    for (var idx=0; idx<assoc_stream._spdyState.pushes.length; idx++) {
        if (assoc_stream._spdyState.pushes[idx]._spdyState.id == id){
            break;
        }
    }
    assoc_stream._spdyState.pushes.splice(idx, 1);
  }
};
