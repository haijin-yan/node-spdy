...
Stream.prototype._handleClose = function _handleClose() {
  var state = this._spdyState;
  console.log("ELE-LL stream handleclose ", state.id);
...
Stream.prototype.destroy = function destroy(error) {
  ...
  console.log("ELE-LL stream destroy ", state.id, " tot ", this.connection._spdyState.counters.streamCount);
...
Stream.prototype._writeData = function _writeData(fin, buffer, cb, chunked) {
  ...
  if (!fin && !chunked && state.ending) {
    if (utils.isLegacy) {
      ...
      fin = this._writableState.length === buffer.length;
    }
  }
  if (!chunked && fin)
    state.ended = true;
...
//
// ### function _recvEnd (callback)
// #### @callback {Function}
// Receive FIN frame
//
Stream.prototype._recvEnd = function _recvEnd(callback) {
  var state = this._spdyState;
